#include "alert.h"

using namespace roo_display;

// Image file ic_filled_36_alert_add_alert 36x36, 4-bit Alpha,  RLE, 176 bytes.
static const uint8_t ic_filled_36_alert_add_alert_data[] PROGMEM = {
  ,
};

const RleImage4bppxBiased<Alpha4, ProgMemPtr>& ic_filled_36_alert_add_alert() {
  static RleImage4bppxBiased<Alpha4, ProgMemPtr> value(
      Box(4, 3, 31, 34), Box(0, 0, 35, 35),
      ic_filled_36_alert_add_alert_data, Alpha4(color::Black));
  return value;
}

// Image file ic_filled_36_alert_auto_delete 36x36, 4-bit Alpha,  RLE, 232 bytes.
static const uint8_t ic_filled_36_alert_auto_delete_data[] PROGMEM = {
  ,
};

const RleImage4bppxBiased<Alpha4, ProgMemPtr>& ic_filled_36_alert_auto_delete() {
  static RleImage4bppxBiased<Alpha4, ProgMemPtr> value(
      Box(1, 1, 34, 34), Box(0, 0, 35, 35),
      ic_filled_36_alert_auto_delete_data, Alpha4(color::Black));
  return value;
}

// Image file ic_filled_36_alert_error_outline 36x36, 4-bit Alpha,  RLE, 253 bytes.
static const uint8_t ic_filled_36_alert_error_outline_data[] PROGMEM = {
  ,
};

const RleImage4bppxBiased<Alpha4, ProgMemPtr>& ic_filled_36_alert_error_outline() {
  static RleImage4bppxBiased<Alpha4, ProgMemPtr> value(
      Box(3, 3, 32, 32), Box(0, 0, 35, 35),
      ic_filled_36_alert_error_outline_data, Alpha4(color::Black));
  return value;
}

// Image file ic_filled_36_alert_error 36x36, 4-bit Alpha,  RLE, 149 bytes.
static const uint8_t ic_filled_36_alert_error_data[] PROGMEM = {
  ,
};

const RleImage4bppxBiased<Alpha4, ProgMemPtr>& ic_filled_36_alert_error() {
  static RleImage4bppxBiased<Alpha4, ProgMemPtr> value(
      Box(3, 3, 32, 32), Box(0, 0, 35, 35),
      ic_filled_36_alert_error_data, Alpha4(color::Black));
  return value;
}

// Image file ic_filled_36_alert_notification_important 36x36, 4-bit Alpha,  RLE, 121 bytes.
static const uint8_t ic_filled_36_alert_notification_important_data[] PROGMEM = {
  0x74, 0x00, 0x27, 0x77, 0x09, 0xA0, 0x97, 0x75, 0x02, 0xC0, 0x27, 0x74, 0x03, 0xC0, 0x37, 0x72,
  0x81, 0x18, 0xDC, 0x81, 0xD8, 0x17, 0x60, 0x60, 0xEF, 0x90, 0xE0, 0x67, 0x40, 0x7F, 0xD0, 0x77,
  0x20, 0x4F, 0xF0, 0x47, 0x10, 0xDF, 0xF0, 0xC7, 0x04, 0xE0, 0x72, 0x07, 0xE0, 0x46, 0x09, 0xE0,
  0x72, 0x07, 0xE0, 0x96, 0x0C, 0xE0, 0x72, 0x07, 0xE0, 0xC6, 0x0E, 0xE0, 0x72, 0x07, 0xE0, 0xE6,
  0xF0, 0x72, 0x07, 0xF6, 0xF0, 0x72, 0x07, 0xF6, 0xFF, 0xC6, 0xFF, 0xC6, 0xFF, 0xC6, 0xF0, 0x72,
  0x07, 0xF6, 0xF0, 0x72, 0x07, 0xF6, 0xF0, 0x72, 0x07, 0xF5, 0x07, 0xFF, 0xC0, 0x73, 0x07, 0xFF,
  0xE8, 0x17, 0x07, 0xFF, 0xF9, 0x07, 0xFF, 0xFB, 0x80, 0xA4, 0x77, 0x77, 0x75, 0x0D, 0xC0, 0xD7,
  0x74, 0x08, 0xC0, 0x87, 0x75, 0x82, 0x8D, 0xD8, 0x73,
};

const RleImage4bppxBiased<Alpha4, ProgMemPtr>& ic_filled_36_alert_notification_important() {
  static RleImage4bppxBiased<Alpha4, ProgMemPtr> value(
      Box(6, 3, 29, 32), Box(0, 0, 35, 35),
      ic_filled_36_alert_notification_important_data, Alpha4(color::Black));
  return value;
}

// Image file ic_filled_36_alert_warning_amber 36x36, 4-bit Alpha,  RLE, 223 bytes.
static const uint8_t ic_filled_36_alert_warning_amber_data[] PROGMEM = {
  ,
};

const RleImage4bppxBiased<Alpha4, ProgMemPtr>& ic_filled_36_alert_warning_amber() {
  static RleImage4bppxBiased<Alpha4, ProgMemPtr> value(
      Box(1, 3, 34, 31), Box(0, 0, 35, 35),
      ic_filled_36_alert_warning_amber_data, Alpha4(color::Black));
  return value;
}

// Image file ic_filled_36_alert_warning 36x36, 4-bit Alpha,  RLE, 148 bytes.
static const uint8_t ic_filled_36_alert_warning_data[] PROGMEM = {
  ,
};

const RleImage4bppxBiased<Alpha4, ProgMemPtr>& ic_filled_36_alert_warning() {
  static RleImage4bppxBiased<Alpha4, ProgMemPtr> value(
      Box(1, 3, 34, 31), Box(0, 0, 35, 35),
      ic_filled_36_alert_warning_data, Alpha4(color::Black));
  return value;
}
